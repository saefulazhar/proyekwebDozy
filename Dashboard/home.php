...
// Query untuk menghitung jumlah tugas berdasarkan kategori
$high = $conn->query("SELECT COUNT(*) AS count FROM task WHERE priority = 'high'")->fetch_assoc()['count'];
$medium = $conn->query("SELECT COUNT(*) AS count FROM task WHERE priority = 'medium'")->fetch_assoc()['count'];
$low = $conn->query("SELECT COUNT(*) AS count FROM task WHERE priority = 'low'")->fetch_assoc()['count'];
$notYet = $conn->query("SELECT COUNT(*) AS count FROM task WHERE status = 'not_started'")->fetch_assoc()['count'];
$in_progress = $conn->query("SELECT COUNT(*) AS count FROM task WHERE status = 'in_progress'")->fetch_assoc()['count'];
$done = $conn->query("SELECT COUNT(*) AS count FROM task WHERE status = 'completed'")->fetch_assoc()['count'];

// Query untuk mengambil data tugas berdasarkan bulan saat ini
$tasks = $conn->query("SELECT task_id, title, description, due_date, priority FROM task WHERE MONTH(due_date) = $currentMonth AND YEAR(due_date) = $currentYear");
?>
    <div class="container">
        <!-- Sidebar -->
        <div class="sidebar">
            ...
            <div class="tasks">
                ...
                <table>
                    ...
                    <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            ...
                                            <button class="delete" onclick="if(confirm('Are you sure you want to delete this task?')) window.location.href='../ScheduleTask/delete_task.php?task_id=<?php echo $task['task_id']; ?>'">Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5" style="text-align: center;">No tasks available for this month.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
